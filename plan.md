...
- [x] Implement the shared calculation foundation: explicit `Single project`, `Workweek`, and `Custom period` state; scenario-vector and component types; validation; unrounded aggregation; coverage tracking; result-state distinctions; regional electricity conversion; source lookup; and stable display formatting.
- [x] Migrate existing text-prompt rows to selected-period semantics and the shared result contract. Preserve model selection, editable usage presets, comparison behavior, lifestyle context, citations, and cited reporting where compatible with the specification.
- [x] Implement Feature 1, multimodal AI generation: repeatable image/video rows, supported-setting controls, the approved image `Not estimated` fallback, frozen EcoLogits video method, component results, unknown-model scenarios, source warnings, professional-AI aggregation, and image-water exclusions.
- [x] Implement Feature 2, project-based AI workload: measured and estimated modes, editable presets, period arithmetic, retry-adjusted calls, recorded-but-unmodeled token fields, duplicate-entry warnings, project grouping, EcoLogits request calculations, and professional-AI aggregation.
- [x] Implement Feature 3, full-meeting footprint: unknown/mixed and detailed endpoint modes, participant-versus-endpoint validation, whole-meeting labeling, duplicate-entry warning, benchmark calculations, broader-digital aggregation, and water exclusion.
- [x] Implement Feature 4, digital media and recreation: shared activity rows plus type-specific streaming, generic social, local-gaming, and cloud-gaming controls; local-device/display accounting; supported infrastructure components; explicit embodied gaps; broader-digital aggregation; and water exclusions.
- [x] Implement Feature 5, assumption and uncertainty scenarios: synchronized global scenario controls, subtotals and combined totals, partial-water coverage, AI share, one-at-a-time carbon sensitivity overrides and ranking, qualitative water/unquantified drivers, and persistent non-confidence language.
- [x] Update the cited report, model-comparison view, reset/share behavior, methodology, and lifestyle context so none restore daily annualization, conflict with the selected period, omit coverage limitations, or use unsupported impact badges or judgments.
- [x] Complete interface integration: field-specific validation, recalculation announcements, keyboard and focus behavior, source disclosure, progressive help, add/edit/remove consistency, and responsive desktop/narrow layouts.
- [x] At each completed checkpoint, review the diff for scope and secrets, run the accumulated harness, keep `spec.md` and this plan aligned with approved revisions, and create a meaningful commit only after the checkpoint is proven.

### Verification

- [x] Run the browser-native automated harness in headless Microsoft Edge and record its output. Cover calculation fixtures for the five features plus text regressions, conversions, aggregation, scenario switching, partial coverage, zero and invalid states, add/edit/remove behavior, period changes, stable sensitivity ranking, and report wording.
- [x] Open the calculator locally with networking unavailable and confirm that calculation, source disclosure, and the test harness work without remote APIs or package installation.
...
- [x] Trace representative displayed values through row components, aggregation, and source IDs; confirm calculations use unrounded values and that carbon and water remain separate.
...
- [x] Commit the completed implementation checkpoint and export the implementation chat transcripts
...
| September 21, 2026 | Continue the supported build while image-generation rows report `Not estimated` for electricity, carbon, and water; retain the source-gap explanation and author-data request. | No companion image dataset was present. This preserves the approved no-inference constraint while allowing video generation and the other four features to proceed. | Approved by the user on September 21, 2026 |
...
- Kept the overall source-freeze checklist item open because the approved image table remains unresolved. The image interface records the activity and explains the gap, while numerical image calculations remain on hold until that source gap is closed.

### September 21, 2026 completed feature integration and automated verification

- Added repeatable multimodal, project-workload, whole-meeting, streaming, generic-social, and local/cloud-gaming rows while preserving the recovered text calculator. Added Alex, Jordan, and Robin reference workflows for repeatable review.
- Added synchronized lower/central/upper views, professional-AI and broader-digital subtotals, combined carbon, supported partial water, AI share, coverage notes, separate operational/embodied presentation where supported, and one-at-a-time carbon sensitivity with qualitative water and unquantified gaps.
- Added seven complete coefficient-group source records with stable identifiers, units, boundaries, titles/links, dates, pinned versions, geography, scenario roles, derivations, evidence labels, and limitations. Image inputs remain explicitly pending under the approved revision.
- Updated cited reporting, reset, and share disclosure. Reset clears expanded rows; the existing share link is explicitly labeled as text-use-only because expanded saved/shareable state remains out of scope.
- Headless Microsoft Edge passed all 26 automated fixtures. The same harness passed with external hosts mapped to `0.0.0.0`, confirming offline calculation and locally packaged source disclosure.
- Frozen-source verification passed: hashes match the reviewed EcoLogits 0.11.1 files, all 17 video models resolve to eight providers and four hardware configurations, electricity factors match, and no inferred image coefficient is present. `git diff --check` passed and the secret-pattern scan found no matches.
- Automated PNG capture was attempted twice for the three reference workflows; Edge wrote blank images despite successful DOM rendering. Per `AGENTS.md`, no further screenshot variations were attempted. Visual evidence and user verification remain open, and the blank generated files are not accepted as evidence.
